...
def code_to_base64_image(code_snippet, language='python', line_numbers=True):
    try:
...
def process_question_text(text):
    ...
    text = re.sub(r'<code(?:\s+lang=["\'][^"\']+["\'])?>(.*?)</code>', replace_code_block, text, flags=re.DOTALL)
    return text


def extract_question_parts(question_text):
    ...
    for line in parts:
        ...
        if in_answers:
            if line.strip().startswith('ANSWER:'):
                ...
                correct_answer = [x.strip() for x in correct_answer]
            else:
...
def generate_moodle_xml(questions_text):
...
"""
        for letter, answer_text in answers:
            ...
            fraction = "{:.9f}".format(100 / len(correct_answer)) if letter in correct_answer else ("0" if len(correct_answer) == 1 else "-100")
